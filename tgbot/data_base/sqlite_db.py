...
# todo для нормально работы создать файл create_bot.py в котором будет создаваться экземпляр бота. Далее просто испортим файл в файл запуска
...
async def sql_read_all_for_admin():
    return cur.execute('SELECT * FROM bookings').fetchall()
